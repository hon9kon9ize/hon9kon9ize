...
_CHINESE_CHAR_RANGE = r'\u4e00-\u9fff\u3400-\u4dbf\U00020000-\U0002a6df\U0002a700-\U0002b73f\U0002b740-\U0002b81f\U0002b820-\U0002ceaf\uf900-\ufaff\U0002f800-\U0002fa1f'
_CHINESE_CHAR_START = re.compile(
    rf'^[{_CHINESE_CHAR_RANGE}]')
_CHINESE_CHAR_END = re.compile(
    rf'[{_CHINESE_CHAR_RANGE}]$')
_PUNCTUATION_RANGE = '＂＃＄％＆＇（）＊＋，－／：；＜＝＞＠［＼］＾＿｀｛｜｝～｟｠｢｣､\u3000、〃〈〉《》「」『』【】〔〕〖〗〘〙〚〛〜〝〞〟〰〾〿–—‘’‛“”„‟…‧﹏﹑﹔·．！？｡。'
_PUNCTUATION_START = re.compile(
    rf'^[{_PUNCTUATION_RANGE}]')
_PUNCTUATION_END = re.compile(
    rf'[{_PUNCTUATION_RANGE}]$')
_FIX_CHINESE_WHITE_SPACE = re.compile(
    "(?<![a-zA-Z]{2})(?<=[a-zA-Z]{1})[ ]+(?=[a-zA-Z] |.$)|(?<=\p{Han}) +")
regex = re.compile(
...
def fix_chinese_text_generation_space(text):
    # attempted regex solution but did work
    # https://stackoverflow.com/questions/64852118/remove-all-spaces-for-chinese-characters-while-keeping-necessary-spaces-for-engl
    lines = text.split("\n")
    output_text = ""
    ...
    for i in range(len(lines)):
        ...
        chars = re.split('\s', line)

        for j in range(len(chars)):
            char = chars[j]
            next_char = chars[j + 1] if j + 1 < len(chars) else None

            if _CHINESE_CHAR_END.search(char) or _PUNCTUATION_END.search(char):
                if next_char != None and (_CHINESE_CHAR_START.search(next_char) or _PUNCTUATION_START.search(next_char)):
                    output_text += char
                else:
                    output_text += char + " "
            else:
...
if __name__ == "__main__":
    ...
    #     )
    # )
    print(fix_chinese_text_generation_space(
        "I like banana 我 Apple ( $ 1, 000 ) 近 近 年 成 為 好 多 港 人 熱 門 移 居 地 嘅 英 國 中 部 城 巿 諾 定 咸 （ 又 譯 諾 丁 漢 ，Nottingham ） ， 多 年 來 一 直 面 對 財 政 困 境 ， 市 議 會 周 三 （11 月29 號 ） 宣 佈 破 產 ， 係 繼 英 國 第 二 大 城 市 伯 明 翰 今 年9 月 宣 佈 破 產 後 ， 近 期「 爆 煲」 嘅 另 一 個 英 國 主 要 城 市 。 諾 定 咸 除 咗 維 持 法 例 規 定 必 須 提 供 嘅 服 務 外 ， 巿 政 府 將 暫 停 所 有 非 必 要 嘅 公 共 開 支 。\n1. 你\n2. 我"))
